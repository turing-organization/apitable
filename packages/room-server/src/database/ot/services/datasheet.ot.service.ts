  ...
  async analyseOperates(
    ...
    if (resultSet.toCreateRecord.size || resultSet.toUnarchiveRecord.size) {
      ...
      if (subscribeInfo.maxRowsPerSheet >= 0 && afterCreateCountInDst > subscribeInfo.maxRowsPerSheet) {
        console.warn(`[WARN] ultrapassando limite de registros por planilha (${afterCreateCountInDst}/${subscribeInfo.maxRowsPerSheet})`);
        // Não lança erro, não bloqueia
      }

      if (subscribeInfo.maxRowsInSpace >= 0 && afterCreateCountInSpace > subscribeInfo.maxRowsInSpace) {
        console.warn(`[WARN] ultrapassando limite total de registros no espaço (${afterCreateCountInSpace}/${subscribeInfo.maxRowsInSpace})`);
        // Não lança erro, não bloqueia
      }
